...
def checkConfig():
	db = dbconfig()
	cursor = db.cursor()
	query = "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA  WHERE SCHEMA_NAME = 'SCM'"
...
def make():
	...
	cursor = db.cursor()
	try:
		cursor.execute("CREATE DATABASE SCM")
	...
	printc("[green][+][/green] Database 'SCM' created")

	# Create tables
	query = "CREATE TABLE SCM.secrets (masterkey_hash TEXT NOT NULL, device_secret TEXT NOT NULL)"
	res = cursor.execute(query)
	printc("[green][+][/green] Table 'secrets' created ")

	query = "CREATE TABLE SCM.entries (sitename TEXT NOT NULL, siteurl TEXT NOT NULL, email TEXT, username TEXT, password TEXT NOT NULL)"
	res = cursor.execute(query)
	...
	# Add them to db
	query = "INSERT INTO SCM.secrets (masterkey_hash, device_secret) values (%s, %s)"
	val = (hashed_mp, ds)
...
if __name__ == "__main__":
	...
	if len(sys.argv)!=2:
		print("Usage: python settings.py <make/delete/remake>")
		sys.exit(0)
