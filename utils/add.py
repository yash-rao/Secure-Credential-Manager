...
def checkEntry(sitename, siteurl, email, username):
	db = dbconfig()
	cursor = db.cursor()
	query = f"SELECT * FROM SCM.entries WHERE sitename = '{sitename}' AND siteurl = '{siteurl}' AND email = '{email}' AND username = '{username}'"
...
def addEntry(mp, ds, sitename, siteurl, email, username):
	...
	db = dbconfig()
	cursor = db.cursor()
	query = "INSERT INTO SCM.entries (sitename, siteurl, email, username, password) values (%s, %s, %s, %s, %s)"
